...
`.github/workflows/update-sports-data.yml` runs manually or daily at 05:17 UTC as a durable fallback snapshot. In the browser, `assets/js/data-source.js` polls the public ESPN scoreboard JSON endpoints every 15 seconds for Premier League, La Liga, and UEFA Champions League matches. It falls back to the latest generated JSON when ESPN has no response. The workflow calls `scripts/update_data.py`, which fetches fixture metadata from TheSportsDB's documented free v1 API and reads player-related headlines from the official BBC Sport, Sky Sports, and ESPN RSS feeds. It stores only a title, a limited excerpt, publication date, source label, and original link; it does not copy full third-party articles, images, or create an external-news page for every item. The workflow commits the refreshed JSON and updates sitemap dates, after which GitHub Pages publishes the new snapshot.
...
The ESPN scoreboard endpoint used by the live browser view currently responds with CORS enabled and a short cache window, but it is a public endpoint rather than a contractual API product. It may change or rate-limit traffic without notice; if that happens, the generated snapshot remains available. The RSS feeds are also public publisher feeds and may change or disappear. GlobalScore validates source hosts, removes duplicate links, limits the external player stream to six stories, and keeps original editorial analysis separate from source-linked headlines. If a future provider requires a secret key, place it in GitHub Actions Secrets and read it from the workflow environment; never put it in HTML or client-side JavaScript.
