...
def generic_player(name, slug, espn_id, team):
    return {
        'name': name,
        'slug': slug,
        'espn_id': espn_id,
        'league_code': 'eng.1',
        'team': team,
        'title': {lang: f'{name}: verified profile and performance' if lang == 'en' else f'{name}: الملف والأداء الموثق' if lang == 'ar' else f'{name} : profil et performances vérifiés' if lang == 'fr' else f'{name}: perfil y rendimiento verificados' for lang in LANGS},
        'description': {lang: f'GlobalScore profile for {name}, with source-linked performance indicators from ESPN when available.' if lang == 'en' else f'صفحة {name} على GlobalScore، مع مؤشرات أداء مرتبطة بمصدر ESPN عند توفرها.' if lang == 'ar' else f'Profil GlobalScore de {name}, avec des indicateurs de performance liés à ESPN lorsqu’ils sont disponibles.' if lang == 'fr' else f'Perfil de GlobalScore de {name}, con indicadores de rendimiento enlazados a ESPN cuando están disponibles.' for lang in LANGS},
        'intro': {lang: f'This page identifies {name} with {team} and displays only source-backed performance data. Original analysis will be added when it can be supported by a verifiable source.' if lang == 'en' else f'تعرّف هذه الصفحة بـ{name} لاعباً في {team} وتعرض فقط بيانات الأداء المدعومة بالمصدر. ستُضاف التحليلات الأصلية عندما يمكن دعمها بمصدر قابل للتحقق.' if lang == 'ar' else f'Cette page identifie {name} avec {team} et n’affiche que des données de performance appuyées par une source. Une analyse originale sera ajoutée lorsqu’elle pourra être vérifiée.' if lang == 'fr' else f'Esta página identifica a {name} con {team} y muestra solo datos de rendimiento respaldados por fuentes. El análisis original se añadirá cuando pueda verificarse.' for lang in LANGS},
        'sections': {lang: [(('Source boundary' if lang == 'en' else 'حدود المصدر' if lang == 'ar' else 'Limites de la source' if lang == 'fr' else 'Límites de la fuente'), ('The performance panel uses the public ESPN athlete profile when available. It does not infer injuries, line-ups, ratings or advanced values that the source does not return.' if lang == 'en' else 'تستخدم لوحة الأداء ملف اللاعب العام في ESPN عند توفره، ولا تستنتج الإصابات أو التشكيلات أو التقييمات أو القيم المتقدمة التي لا يرسلها المصدر.' if lang == 'ar' else 'Le panneau utilise le profil public ESPN lorsqu’il est disponible. Il ne déduit ni blessures, ni compositions, ni notes, ni valeurs avancées absentes de la source.' if lang == 'fr' else 'El panel utiliza el perfil público de ESPN cuando está disponible. No infiere lesiones, alineaciones, valoraciones ni valores avanzados que la fuente no devuelva.'))] for lang in LANGS},
    }


PLAYERS = [PLAYER, generic_player('Bukayo Saka', 'bukayo-saka', '280555', 'Arsenal'), generic_player('Erling Haaland', 'erling-haaland', '253989', 'Manchester City')]
...
def search_page(lang):
    ...
    extra = f'<script>window.GLOBAL_LANG = "{lang}";</script><script type="module" src="../../assets/js/search-page.js?v=2"></script>'
    return document(lang, c['title_search'], c['desc_search'], canonical, path, body, schema, extra)


def player_page(lang, player=PLAYER):
    c = COPY[lang]
...
def compare_page(lang):
    c = COPY[lang]
    path = 'compare/players.html'
    canonical = f'{BASE}/{lang}/{path}'
    title = c['compare_title']
    description = c['compare_desc']
    body = f'''{common_nav(lang)}<main class="page-shell discovery-page"><a class="article-back" href="../players/">← {esc(c['players'])}</a><section class="discovery-hero compare-hero"><span class="section-kicker">{esc(c['compare'])}</span><h1>{esc(title)}</h1><p>{esc(description)}</p></section><section class="performance-panel compare-panel" id="playerCompare"><form id="playerCompareForm" class="compare-controls"><label><span>{esc(c['player_one'])}</span><select id="comparePlayerOne" aria-label="{esc(c['player_one'])}"></select></label><span class="compare-vs" aria-hidden="true">VS</span><label><span>{esc(c['player_two'])}</span><select id="comparePlayerTwo" aria-label="{esc(c['player_two'])}"></select></label><button class="primary-btn" type="submit">{esc(c['compare_button'])}</button></form><div id="playerCompareStatus" class="performance-status">{esc(c['compare_loading'])}</div><div class="compare-player-grid"><div id="comparePlayerCardOne" class="compare-player-card"></div><div id="comparePlayerCardTwo" class="compare-player-card"></div></div><div class="performance-visuals"><div class="performance-chart-card"><h2>{esc(c['performance_chart'])}</h2><div id="playerCompareBars" class="performance-bars compare-bars"></div></div><div class="performance-radar-card"><h2>{esc(c['performance_profile'])}</h2><div id="playerCompareRadar" class="performance-radar"></div></div></div><p class="performance-source compare-disclaimer">{esc(c['compare_note'])}</p><p id="playerCompareSource" class="performance-source"></p></section></main><footer class="footer"><div class="footer-inner"><a class="brand footer-brand" href="../"><img class="logo-img" src="../../assets/img/logo.svg" alt=""><span>Global<span class="brand-accent">Score</span></span></a><p>GlobalScore Editorial</p></div></footer>'''
    schema = json.dumps({'@context': 'https://schema.org', '@type': 'WebPage', 'name': title, 'description': description, 'url': canonical, 'isAccessibleForFree': True}, ensure_ascii=False)
    extra = f'<script>window.GLOBAL_LANG = "{lang}";</script><script type="module" src="../../assets/js/player-compare.js?v=2"></script>'
    return document(lang, title, description, canonical, path, body, schema, extra)
...
def players_index(lang, articles):
    ...
    canonical = f'{BASE}/{lang}/{path}'
    player_cards = ''.join(f'<a class="discovery-card player-card" href="{player["slug"]}.html"><div class="player-identity"><span class="team-page-crest">⚽</span><strong>{esc(player["name"])}</strong></div><p>{esc(player["intro"][lang])}</p><span class="news-arrow">→</span></a>' for player in PLAYERS)
    source_cards = []
...
def main():
    ...
    urls = []
    for lang in LANGS:
        for subdir in ('search', 'players', 'compare'):
        ...
        for player in PLAYERS:
            (ROOT / lang / f'players/{player["slug"]}.html').write_text(player_page(lang, player), encoding='utf-8')
        (ROOT / lang / 'compare/players.html').write_text(compare_page(lang), encoding='utf-8')
        urls.extend([f'{BASE}/{lang}/search/index.html', f'{BASE}/{lang}/players/index.html', *[f'{BASE}/{lang}/players/{player["slug"]}.html' for player in PLAYERS], f'{BASE}/{lang}/compare/players.html'])
