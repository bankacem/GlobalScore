...
HUB_TEMPLATE_VERSION = '2026-08-matchday-hub-v1'
HUB_COPY = {
    'en': {'title': "Football matchday guide: today's fixtures and key questions", 'description': "A clear guide to today's football fixtures, kick-off times and the tactical questions worth watching.", 'heading': "Today's football fixtures", 'intro': "This matchday guide brings the available fixtures into one useful page. Each match has its own preview, with the published time, competition and questions to follow before kick-off.", 'list': 'Match previews', 'note': 'Times and fixture details are based on the available free scoreboard snapshot. Line-ups, referees and live events are added only when officially published.', 'back': 'Open live scores and match details'},
    'ar': {'title': 'دليل مباريات اليوم: المواعيد وأهم الأسئلة', 'description': 'دليل واضح لمباريات كرة القدم اليوم، مواعيد البداية والأسئلة التكتيكية التي تستحق المتابعة.', 'heading': 'مباريات كرة القدم اليوم', 'intro': 'يجمع هذا الدليل مباريات اليوم المتاحة في صفحة واحدة مفيدة. وتضم كل مواجهة معاينة مستقلة تتضمن الموعد المعلن والمسابقة والأسئلة التي تستحق المتابعة قبل البداية.', 'list': 'معاينات المباريات', 'note': 'تعتمد المواعيد وبيانات المباريات على snapshot المجاني المتاح. ولا تُضاف التشكيلات والحكام والأحداث الحية إلا عند نشرها رسمياً.', 'back': 'فتح النتائج المباشرة وتفاصيل المباريات'},
    'fr': {'title': 'Guide de la journée : matchs du jour et points clés', 'description': 'Un guide clair des matchs de football du jour, des horaires et des questions tactiques à suivre.', 'heading': 'Les matchs de football du jour', 'intro': 'Ce guide rassemble les rencontres disponibles dans une page pratique. Chaque match possède son avant-match avec l’horaire publié, la compétition et les questions à observer avant le coup d’envoi.', 'list': 'Avant-matchs', 'note': 'Les horaires et les affiches reposent sur le snapshot gratuit disponible. Les compositions, arbitres et événements ne sont ajoutés qu’après leur publication officielle.', 'back': 'Ouvrir les scores en direct et les détails'},
    'es': {'title': 'Guía de la jornada: partidos de hoy y claves', 'description': 'Una guía clara de los partidos de fútbol de hoy, sus horarios y las preguntas tácticas que conviene seguir.', 'heading': 'Partidos de fútbol de hoy', 'intro': 'Esta guía reúne los encuentros disponibles en una sola página útil. Cada partido cuenta con una previa propia, el horario publicado, la competición y las preguntas que conviene observar antes del inicio.', 'list': 'Previas de los partidos', 'note': 'Los horarios y los encuentros se basan en el snapshot gratuito disponible. Las alineaciones, los árbitros y los acontecimientos solo se añaden cuando se publican oficialmente.', 'back': 'Abrir el marcador en directo y los detalles'},
}


def matchday_hub_page(lang):
    c = COPY[lang]
    hub = HUB_COPY[lang]
    ...
    schema = json.dumps({'@context': 'https://schema.org', '@type': 'CollectionPage', 'name': hub['title'], 'description': hub['description'], 'url': canonical, 'inLanguage': lang, 'datePublished': f'{hub_date}T00:00:00Z', 'dateModified': hub_date, 'isAccessibleForFree': True, 'publisher': {'@type': 'Organization', 'name': 'GlobalScore', 'url': BASE}}, ensure_ascii=False)
    return f'''<!doctype html><html lang="{lang}" dir="{c['dir']}" prefix="og: https://ogp.me/ns#"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{escape(hub['title'])} | GlobalScore</title><meta name="description" content="{escape(hub['description'])}"><meta name="robots" content="index,follow"><link rel="canonical" href="{canonical}">{hreflang}<link rel="alternate" hreflang="x-default" href="{BASE}/en/articles/{hub_slug}.html"><link rel="stylesheet" href="../../assets/css/critical.css?v=4"><link rel="stylesheet" href="../../assets/css/main.css?v=9"><meta property="og:type" content="article"><meta property="og:site_name" content="GlobalScore"><meta property="og:title" content="{escape(hub['title'])}"><meta property="og:description" content="{escape(hub['description'])}"><meta property="og:url" content="{canonical}"><meta property="article:published_time" content="{hub_date}T00:00:00Z"><script type="application/ld+json">{schema}</script></head><body><header class="site-header"><div class="header-container"><a class="brand" href="../../{lang}/"><img class="logo-img" src="../../assets/img/logo.svg" alt=""><span>Global<span class="brand-accent">Score</span></span></a><nav class="nav"><a class="nav-link" href="../../{lang}/">{escape(c['back'])}</a><a class="nav-link" href="../../{lang}/articles/">{escape(c['index'])}</a></nav><div class="controls"><a class="language-btn" href="../../ar/articles/{hub_slug}.html">AR</a><a class="language-btn" href="../../en/articles/{hub_slug}.html">EN</a><a class="language-btn" href="../../fr/articles/{hub_slug}.html">FR</a><a class="language-btn" href="../../es/articles/{hub_slug}.html">ES</a></div></div></header><main class="page-shell"><article class="article-page"><header class="article-header"><a class="article-back" href="../../{lang}/">← {escape(c['back'])}</a><span class="section-kicker">{escape(c['tag'])}</span><h1>{escape(hub['title'])}</h1><p class="article-dek">{escape(hub['description'])}</p><div class="article-meta"><span>{escape(c['read'])}</span><time datetime="{hub_date}">{escape(hub_date)}</time><span>GlobalScore Editorial</span></div></header><div class="article-content"><p>{escape(hub['intro'])}</p><h2>{escape(hub['heading'])}</h2><ul class="article-checklist matchday-hub-list">{items}</ul><p class="article-note">{escape(hub['note'])}</p><p><a class="primary-btn" href="../../{lang}/#today">{escape(hub['back'])} <span>→</span></a></p></div></article></main><footer class="footer"><div class="footer-inner"><a class="brand footer-brand" href="../../{lang}/"><img class="logo-img" src="../../assets/img/logo.svg" alt=""><span>Global<span class="brand-accent">Score</span></span></a><p>GlobalScore Editorial</p><div class="footer-links"><a href="../../ar/articles/{hub_slug}.html">العربية</a><a href="../../en/articles/{hub_slug}.html">English</a><a href="../../fr/articles/{hub_slug}.html">Français</a><a href="../../es/articles/{hub_slug}.html">Español</a></div></div></footer></body></html>'''
...
def page_for(fixture, lang):
    ...
    internal_match = f'../../{lang}/matches/{fixture["event_id"]}.html' if fixture.get('event_id') else internal_score
    internal_home = f'../../{lang}/teams/{slugify(fixture["home"])}.html'
    internal_away = f'../../{lang}/teams/{slugify(fixture["away"])}.html'
    internal_league = f'../../{lang}/leagues/{slugify(fixture["league"]["en"])}.html'
    ...
    body = f'<p>{escape(CONTEXT[lang])}</p><p>{escape(METHOD[lang])}</p><h2>{escape(lens_heading)}</h2><p>{escape(LENS[lang])}</p><p>{escape(ANGLES[lang][fixture["angle"] % len(ANGLES[lang])])}</p><h2>{escape(watch_heading)}</h2><ul class="article-checklist">{watch_items}</ul><h2>{escape(c["questions"])}</h2><ol>{questions}</ol><h2>{escape(c["follow"])}</h2><p>{escape(follow_text)}</p><nav class="article-links" aria-label="Related GlobalScore links"><a href="{internal_match}">{escape(c["match_page"])}</a><a href="{internal_home}">{escape(c["home_page"])}</a><a href="{internal_away}">{escape(c["away_page"])}</a><a href="{internal_league}">{escape(c["league_page"])}</a><a href="{internal_table}">{escape(c["table"])}</a><a href="{internal_index}">{escape(c["index"])}</a></nav><h2>{escape(c["sources"])}</h2><p>{escape(source_text)}</p><p class="article-note"><a href="{escape(external_source)}" rel="noopener noreferrer" target="_blank">{escape(c["source_link"])}</a> · <a href="https://www.espn.com/soccer/scoreboard" rel="noopener noreferrer" target="_blank">ESPN football scoreboard</a></p>'
...
skipped_existing = 0
hub_created_or_updated = 0
hub_skipped_existing = 0
...
hub_needs_write = os.getenv('GLOBAL_SCORE_FORCE_ARTICLES') == '1' or hub_record.get('templateVersion') != HUB_TEMPLATE_VERSION or not all(path.exists() for path in hub_paths)
if hub_needs_write:
    for lang, article_path in zip(LANGS, hub_paths):
        article_path.parent.mkdir(parents=True, exist_ok=True)
        article_path.write_text(matchday_hub_page(lang), encoding='utf-8')
    hub_record['lastGenerated'] = now
    hub_created_or_updated = 1
else:
    hub_skipped_existing = 1
registry[hub_key] = {'eventId': '', 'slug': hub_slug, 'date': hub_date, 'home': 'GlobalScore', 'away': 'Matchday guide', 'lastGenerated': hub_record.get('lastGenerated', now), 'languages': list(LANGS), 'templateVersion': HUB_TEMPLATE_VERSION, 'type': 'matchday-hub'}
...
new_urls |= {f'{BASE}/{lang}/articles/{hub_slug}.html' for lang in LANGS}
...
print(f'matchday_hub {"created_or_updated" if hub_created_or_updated else "skipped_existing"} x {len(LANGS)} languages = {(hub_created_or_updated or hub_skipped_existing) * len(LANGS)} pages')
